...
const minterAddedEventSchema = new Schema(
  {
...
minterAddedEventSchema.post('save', async (doc) => {
  try {
...
minterAddedEventSchema.post('update', async (doc) => {
  try {
    const user = await UserProfile.findOne({
      userCryptoAddress: doc.result.account,
    });
...
module.exports = model('MinterAddedEvent', minterAddedEventSchema);
