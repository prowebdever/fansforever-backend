...
contract AuctionEngineV6 is TRC721TokenReceiver, TRC165 {
    ...
    function cancelAuction(uint256 auctionIndex) public {
        ...
        require(creator == msg.sender, "you are not auction creator!");
        TRC721 asset = TRC721(auction.assetAddress);
        asset.transferFrom(address(this), msg.sender, auction.assetId);
    }

    function acceptBid(uint256 auctionIndex) public {
        Auction storage auction = auctions[auctionIndex];
        address creator = auction.creator;
        require(creator == msg.sender, "you are not auction creator!");
        require(auction.currentBidAmount > 0, "there is no bidder");
